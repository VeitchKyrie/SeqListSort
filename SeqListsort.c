...
void SelectSort_OP(SeqList* pSeq)//选择排序优化版
{
	...
	while (start <= end)
	{
		...
		if (min != start)
		{
			...
			if (max == end)//如果最大值在右边，肯定要被移走，此时要转移到新的相应的位置
				max = min;
		}
		if (max != end)
...
int BinarySearch(SeqList *pSeq, DataType d)//二分查找函数
{
	int left = 0;
	int right = pSeq->sz - 1;//最右边元素的下标
	while (left < right)//循环次数
	{
		int mid = left + (right - left) / 2;//确定中间元素的下标

		if (pSeq->_data[mid]>d)//当中间元素大于d
		{
			right = mid - 1;
		}
		else if (pSeq->_data[mid] < d)//当中间元素小于d
		{
			left = mid + 1;
		}
		else
...
int BinarySearch_R(SeqList *pSeq, int left, int right, DataType d)//二分查找到的递归写法
{
	assert(pSeq);
	int mid = left + (right - left) >> 1;//右移一位相当于除二
	while (left < right)
	{
		if (pSeq->_data[mid] < d)
